...
std::queue<cv_bridge::CvImageConstPtr> ptrList;
std::queue<bool> flagList;
...
void process_image() {
    ...
    while(ros::ok) {
        // ROS_WARN("ptrList size: %d", ptrList.size());
        if(!ptrList.empty()) {
            cv_bridge::CvImageConstPtr ptr = ptrList.front();
            PUB_THIS_FRAME = flagList.front();
            ptrList.pop();

            cv::Mat show_img = ptr->image;
            TicToc t_r;
            double cur_img_time = ptr->header.stamp.toSec();
            uint cur_seq = ptr->header.seq;
        ...
        if (PUB_THIS_FRAME)
        {
                ...
                feature_points->header.stamp = ptr->header.stamp;
                ...
                *depth_cloud_temp = *depthCloud;
                ...
                sensor_msgs::ChannelFloat32 depth_of_points = depthRegister->get_depth(ptr->header.stamp, show_img, depth_cloud_temp, trackerData[0].m_camera, feature_points->points);
                ...
                if (pub_match.getNumSubscribers() != 0)
                {
                    ...
                    if(PRINT_MATCH_DOTS) {
                        ...
                        //cur_img, prev_img
                        std::string filename = Float2Str(ptr->header.stamp.toSec());
...
int main(int argc, char **argv)
{
    ...
    std::thread processing_image{process_image};
